...
import { useFavorites, useMyBookings, useLogout, initials } from "@/lib/api";
...
function Me() {
  ...
  const completed = bookings?.past.filter((b) => b.booking.status === "completed") ?? [];
  const spent = completed.reduce((sum, b) => sum + b.booking.price, 0);
  const totalBookings = (bookings?.upcoming.length ?? 0) + (bookings?.past.length ?? 0);
  const points = spent * 2;
  ...
  return (
    <AppShell>
      <header className="px-5 pt-6 pb-5 flex items-center gap-4">
        <div className="h-16 w-16 rounded-2xl bg-gradient-to-br from-gold/40 to-gold/10 grid place-items-center font-display text-2xl">{initials(user.name)}</div>
      ...
      <div className="px-5 grid grid-cols-3 gap-2 text-center">
        {[[`${totalBookings}`,"Bookings"],[`$${spent.toLocaleString()}`,"Spent"],[`${points.toLocaleString()}`,"Points"]].map(([v,l])=>(
      ...
      {(favorites?.length ?? 0) > 0 && (
        <section id="favorites" className="mt-6 px-5">
      ...
      <section className="mt-6 px-5">
        <Link to="/app/subscription" className="block rounded-3xl bg-ink text-white p-5 relative overflow-hidden">
          ...
          <div className="text-xs text-white/60 mt-1">Renews monthly · $29.99/mo</div>
        </Link>
      </section>
      ...
      <section className="mt-6 px-5">
        <div className="rounded-2xl border border-border divide-y divide-border">
          {[
            ...
            <button key={r.l} onClick={r.onClick} className="w-full flex items-center gap-3 p-4 text-left">{inner}</button>
          )})}
        </div>
      </section>
      ...
      <button
        ...
        className="mt-4 mx-5 mb-4 w-[calc(100%-2.5rem)] flex items-center justify-center gap-2 py-3 rounded-2xl border border-border text-sm text-destructive disabled:opacity-50"
      >
